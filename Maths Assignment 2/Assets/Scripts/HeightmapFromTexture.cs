...
public class HeightmapFromTexture : MonoBehaviour {
    ...
    public static void heightmapFromTexture (Texture2D heightmap) {
        ...
        if (w2 != w || h != w)
        {
            ...
            if (heightmap.filterMode == FilterMode.Point)
            {
                ...
                for (int y = 0; y < w2; y++)
                {
                    ...
                    int thisY = Mathf.RoundToInt(dy * y) * w;
                    int yw = y * w2;
                    for (int x = 0; x < w2; x++)
                    {
                        map[yw + x] = mapColors[thisY + Mathf.RoundToInt(dx * x)];
                    }
                }
            }
            // Otherwise resize using bilinear filtering
            else if (heightmap.filterMode == FilterMode.Bilinear)
            {
